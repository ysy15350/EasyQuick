...
public class MemberApiImpl implements MemberApi {
    ...
    public void shopRegister(String mobile, String password, String verify, ApiCallBack callBack) {
        ...
        server.setMethodName(moduleName + "shopRegister");
    ...
    public void store_save_info(int icon, String name, String license, String license_icon, String mobile, String address, String business, String description, String pay_password, ApiCallBack callBack) {
        ...
        server.setMethodName(moduleName + "store_save_info");
        ...
        if (!CommFunAndroid.isNullOrEmpty(name)) {
            server.setParam("name", name);
        }
        if (!CommFunAndroid.isNullOrEmpty(license)) {
            server.setParam("license", license);
        }
        if (!CommFunAndroid.isNullOrEmpty(license_icon)) {
            server.setParam("license_icon", license_icon);
        }
        if (!CommFunAndroid.isNullOrEmpty(mobile)) {
            server.setParam("mobile", mobile);
        }
        if (!CommFunAndroid.isNullOrEmpty(address)) {
            server.setParam("address", address);
        }

        if (!CommFunAndroid.isNullOrEmpty(business)) {
            server.setParam("business", business);
        }

        if (!CommFunAndroid.isNullOrEmpty(description)) {
            server.setParam("description", description);
        }
    ...
    public void user_payment(String sid, String price, String remark, ApiCallBack callBack) {
        ...
        server.setParam("uid", sid);
        server.setParam("sid", BaseData.getInstance().getUid());//店铺id
        server.setParam("price", price);
    ...
    public void wx_alipay_code(int uid, int type, float price, String remark, ApiCallBack callBack) {
        ...
        server.setMethodName(moduleName + "wx_alipay_code");
        server.setParam("uid", uid);
        server.setParam("sid", BaseData.getInstance().getUid());
        server.setParam("type", type);//类型  1:支付宝   2:微信
        server.setParam("price", price);
        server.setParam("remark", remark);
    ...
    public void personWater(int page, int pageSize, ApiCallBack callBack) {
        ...
        server.setMethodName(moduleName + "personWater");
        server.setParam("uid", BaseData.getInstance().getUid());
        server.setParam("page", page);
        server.setParam("limit", pageSize);
    ...
    public void shopYonjinWater(int page, int pageSize, ApiCallBack callBack) {
        ...
        server.setMethodName(moduleName + "shopYonjinWater");
        server.setParam("uid", BaseData.getInstance().getUid());
        server.setParam("page", page);
        server.setParam("limit", pageSize);
    ...
    public void checkUserinfo(ApiCallBack callBack) {
        ...
        server.setParam("uid", BaseData.getInstance().getUid());
        server.setParam("type", 1);//type: 1:店铺   2:会员
